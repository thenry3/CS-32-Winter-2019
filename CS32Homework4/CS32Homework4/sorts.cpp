...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
	...
	{
		int count = i;
		while (count > 0 && comp(s[count], s[count - 1]))
		{
			swap(s[count - 1], s[count]);
			count--;
		}
	}
}
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Store*> ptrvector;
		int size = auxStores.size();
		
		for (auto x : auxStores)
			ptrvector.push_back(&x);

		sort(ptrvector.begin(), ptrvector.end(), compareStorePtr);

		for (int i = 0; i < size; i++)
			stores[i] = *ptrvector[i];
