...
public:
	...
	void swap(Set<ItemType>& other) // Exchange the contents of this set with the other one.
	{
		// Swap head pointers
...
private:
	...
	void createEmpty() // Create an empty list.  (Will be called only by constructors.)
	{
		m_size = 0;
	...
	void insertBefore(Node* p, const ItemType& value) // Insert value in a new Node before Node p, incrementing m_size.
	{
		// Create a new node
	...
	void doErase(Node* p) // Remove the Node p, decrementing m_size.
	{
